...
function closeMenu() {
    ...
    // [fix-6] 드래그 중 메뉴가 닫히면 body에 고아로 남은 dragging 엘리먼트 정리
    const $orphan = $(document.body).children('.qpl-row.qpl-dragging');
    if ($orphan.length) $orphan.remove();
...
function setupTouchDrag($list) {
    ...
    let offsetY     = 0;
    let pendingY    = null;
    // [fix-3] pendingX 제거 — 선언 후 미사용 데드코드였음
    let rafId       = null;
    let pointerId   = null;
