...
function setupTouchDrag($list) {
    const list = $list[0];
    let dragging = null;       // 현재 드래그 중인 .qpl-row 엘리먼트
    let placeholder = null;    // 자리 표시용 빈 div
    let startY = 0;
    let offsetY = 0;           // 터치 시작 위치와 요소 top의 차이
    let lastClientY = 0;

    function getRowAt(y) {
        // y 위치에 있는 행 찾기 (드래그 중인 행 제외)
    ...
    function onStart(e) {
        ...
        document.body.appendChild(dragging); // body로 이동 (스크롤 영향 제거)
    }
