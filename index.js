...
// pointerdown/move/up: 터치·마우스 통합, 300ms 딜레이 없음
// transform으로 이동 (GPU 가속, 잔상 없음)
// getBoundingClientRect 최소화 (렉 감소)
function setupTouchDrag($list) {
    ...
    let pendingY   = null;
    let rafId      = null;
    // 행 위치 캐시 (드래그 시작 시 1회 측정)
    let rowCache   = [];

    function buildRowCache() {
        rowCache = [...list.querySelectorAll('.qpl-row:not(.qpl-dragging)')].map(row => ({
            el: row,
            top: row.offsetTop,
    ...
    function getRowAt(listScrollTop, clientY) {
        // list 내 상대 Y 계산
        const listRect = list.getBoundingClientRect();
        const relY = clientY - listRect.top + list.scrollTop;
        for (const r of rowCache) {
            if (relY >= r.top && relY <= r.top + r.height) return r.el;
        }
        return null;
    }
    ...
    function onPointerDown(e) {
        ...
        // dragging 요소 body에 fixed로 붙임
        dragging = row;
        dragging.classList.add('qpl-dragging');
        dragging.style.width     = rowWidth + 'px';
    ...
    function onPointerMove(e) {
        ...
        if (rafId === null) {
            rafId = requestAnimationFrame(() => {
                ...
                // placeholder 재배치
                const target = getRowAt(list.scrollTop, clientY);
                if (target && target !== placeholder) {
                    const tRect  = target.getBoundingClientRect();
                    const middle = tRect.top + tRect.height / 2;
                    if (clientY < middle) {
                        list.insertBefore(placeholder, target);
                    } else {
                        list.insertBefore(placeholder, target.nextSibling);
                    }
                    // 캐시 갱신 (placeholder 이동했으므로)
                    buildRowCache();
                }
            });
        }
    ...
    // pointer events: 터치·마우스 통합, 딜레이 없음
