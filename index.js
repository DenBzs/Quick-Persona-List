...
function addQuickPersonaButton() {
    ...
    $c.append(`
        <div id="qplBtn" class="interactable" tabindex="0" title="페르소나 목록 열기">
...
function setupTouchDrag($list) {
    ...
    let offsetY = 0;
    let pendingY = null;   // rAF용 최신 Y값
    let rafId   = null;    // requestAnimationFrame ID

    function getRowAt(y) {
        const rows = [...list.querySelectorAll('.qpl-row:not(.qpl-dragging)')];
    ...
    function onStart(e) {
        ...
        // 드래그 중 MutationObserver 억제 (렉 방지)
        pauseObserver();
    }
    ...
    // document 리스너는 named function으로 등록해야 cleanup 가능
    // 메뉴 닫힐 때 자동 제거됨 (list 자체가 제거되므로 충분)
