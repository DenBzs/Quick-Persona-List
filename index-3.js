...
// ─── Settings ─────────────────────────────────────────────────────────────────
// 구 확장 폴더명 (마이그레이션 전용)
const OLD_MODULE_NAME = 'Quick-Persona-List';

function getSettings() {
    const ctx = SillyTavern.getContext();

    // 구 이름으로 저장된 데이터가 있고, 새 이름 데이터가 없을 때만 1회 마이그레이션
    if (!ctx.extensionSettings[MODULE_NAME] && ctx.extensionSettings[OLD_MODULE_NAME]) {
        ctx.extensionSettings[MODULE_NAME] = structuredClone(ctx.extensionSettings[OLD_MODULE_NAME]);
        delete ctx.extensionSettings[OLD_MODULE_NAME];
        ctx.saveSettingsDebounced();
        console.log(`[${MODULE_NAME}] 🔄 설정 마이그레이션 완료 (${OLD_MODULE_NAME} → ${MODULE_NAME})`);
    }
